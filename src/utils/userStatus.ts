...
// 5 minutes gives enough buffer for Doze mode (Android) and iOS suspension
// which can delay heartbeats by 3-10+ minutes in background
...
export function isUserOnline(profile: UserStatusProfile): boolean {
  ...
  const fiveMinAgo = Date.now() - FIVE_MINUTES_MS;
  return new Date(profile.last_heartbeat_at).getTime() > fiveMinAgo;
}